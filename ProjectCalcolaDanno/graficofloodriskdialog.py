...
# from PyQt5.QtGui import QAction, QIcon, QApplication, QMessageBox
...
class graficofloodriskDialog(QDialog,FORM_CLASS):

    def __init__(self, iface, idTipo,tipo, parent=None):
        super(graficofloodriskDialog, self).__init__(parent)
        self.iface=iface
        self.idTipo=idTipo
        self.tipo = tipo
        self.setupUi(self)
    ...
    def run(self):
        ...
        self.pushButton.pressed.connect(self.querySql)
...
##                        hold(True)
                ...
                plt.xlabel(self.tr("Water depth (m)"))
                plt.ylabel(self.tr('Damage (%)'))
                posizioneLegend='best'

                # legend(nomiLegenda, posizioneLegend, prop=fontp)
                plt.legend(nomiLegenda)

                plt.suptitle(txt,fontsize=14)
                plt.title(txt1,fontsize=12)
                limy=plt.ylim()
                y1=limy[0]
                y2=limy[1] * 1.1
                lim=[y1,y2]
            ...
            txt= "Unexpected error:  %s " % (exc_obj)
            QMessageBox.information(None, "Warning", txt)
