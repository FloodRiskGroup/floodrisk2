...
def mainCaricaGeodatiFloodRisk(self,FilesList,UpLoad, bar,instance):
    """
    Upload shapefiles into spatialite geodatabase
    ...
    if carico>0:
        ...
        if layer_geom_type==CodTipoGeom:
            geomok=bool('True')
        elif CodTipoGeom==6 and layer_geom_type==3:                 # include POLYGON as MULTIPOLYGON
            geomok=bool('True')
        ...
        #Empty table for the specific instance
    ...
    if carico>0:
        ...
        if numFeatures>0:
            ...
            # pair with fields of the same name does not consider the case sensitive
            for i in range(NumFields):
                for j in range(NumFieldsShp):
                    if str.upper(NameField[i])==str.upper(NameFieldShp[j]):
                        # todo : check the type
