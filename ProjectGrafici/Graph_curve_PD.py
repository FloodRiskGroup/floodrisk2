...
def mainPD(InputList):
    ...
    if len(ListaDamages)>0:
        ...
        plt.grid(True)
        plt.title('CURVE F-N', fontproperties=font2)
##        plt.xlabel('Number of Loss of Lifes', fontproperties=font1)
##        plt.ylabel('Annual Exceedance Probability (AEP)', fontproperties=font1)
    ...
    CurrentScenarioInt=InputList[2]
    sql='SELECT YearReturnPeriod, LOL FROM FreqNumLOL WHERE Numscenario=%d ORDER BY YearReturnPeriod;' % (CurrentScenarioInt)
    cur.execute(sql)
    ListaDamages=cur.fetchall()
...
##        plt.ylabel('Annual Exceedance Probability (AEP)', fontproperties=font1)
    ...
    if (InputList[1] == InputList[2]):
        QMessageBox.information(None, "FloodRisk_2", "Set scenario 1 and 2")
    else:
        plt.show()
